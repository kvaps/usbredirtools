...
LASTNUM=$(virsh --connect=qemu:///system qemu-monitor-command --hmp $VM 'info chardev' | grep -Po -m1 '(?<=charredir)[0-9]*')
...
for USBREDIR in ${USBREDIRS[@]}; do
    case $ACTION in
        ...
        ;;
        disconnect)
            NUM=$(virsh --connect=qemu:///system qemu-monitor-command --hmp $VM 'info chardev' | grep $USBREDIR | grep -Po -m1 '(?<=charredir)[0-9]*')
